...
export async function POST(req: Request) {
   ...
   * For testing the order pipeline (emails, order numbers, inventory)
   * without real payments. Unset the env var to re-enable real
   * payments. The client sees `bypass: true` in the response and
   * skips the Razorpay modal. */
  ...
  // Bypass mode short-circuits with a synthetic id so the client never
  // opens the Razorpay modal — purely for testing the post-payment
  // pipeline (email, order number, inventory, etc.).
  if (bypass) {
    return NextResponse.json({
